...
// Функция возвращает точки пересечения двух окружностей.
// Одна окружность задается this, а вторая - circle2.
Circle2D.prototype.Intersection_TwoCircles = function(circle2)
{
	var midPt = new Point2D();
	var r1 = this.radius;
	var r2 = circle2.radius;
	...
	points[0][0] = midPt[0] - h*(ptCenter2[1] - ptCenter1[1])/d;
	points[0][1] = midPt[1] + h*(ptCenter2[0] - ptCenter1[0])/d;

	// point 2
	points[1] = new Point2D();
	points[1][0] = midPt[0] + h*(ptCenter2[1] - ptCenter1[1])/d;
	points[1][1] = midPt[1] - h*(ptCenter2[0] - ptCenter1[0])/d;
...
Circle2D.prototype.IntersectionLineCircle = function(line, point1, point2)
{
	var pt1 = [];
	var pt2 = [];
	var t_point = new Point2D(this.center[0], this.center[1]);
	if(line.Distance(t_point) > this.radius)
	{
		return null;
	}
...
//  Функция возвращает точки пересечения 
// окружности this и прямой line.
Circle2D.prototype.Intersection_LineCircle = function(line)
{
	var points = [2];
	var t_point = new Point2D(this.center[0], this.center[1]);
	if(line.Distance(t_point) > this.radius)
	...
	if(Math.abs(norm[1]) > 0.00001)   // 0.00001 - очень маленькое значение ( ε )
	{
		...
		points[0] = new Point2D();
		points[0][0] = rez[0];
		points[0][1] = y1 + m * (rez[0] - x1);		
		
		// Точка пересечения 2
		points[1] = new Point2D();
		points[1][0] = rez[1];
		points[1][1] = y1 + m * (rez[1] - x1);
	...
	else if(Math.abs(norm[0]) > 0.00001) // 0.00001 - очень маленькое значение ( ε )
	{
		...
		// Точка пересечения 1
		points[0] = new Point2D();
		points[0][0] = x1 + m * (rez[0] - y1);
		points[0][1] = rez[0];
		
		// Точка пересечения 2
		points[1] = new Point2D();
		points[1][0] = x1 + m * (rez[1] - y1);
		points[1][1] = rez[1];
...
// Возвращает rez - результат решения (массив из двух значений).
...
/*
// Данная функция делает тоже самое, что и функция FACET2, но имеет другое название.
Plane3D.prototype.CreateInclinePlane2 = function(angle, pt1, pt2, pt3)
{
	var norm2d = new Vector2D(pt1[1] - pt2[1], pt2[0] - pt1[0]);
	norm2d.Normer();
...
//  Данная функция отличается от функции CreateInclinePlane2 только другим
// знаком для угла angle. Иногда удобно иметь две функции при наклоне для
// создаваемых в противоположные стороны плоскостей относительно 
// плоскости OXY. Например, при построении огранок алмазов грани (фасеты)
// короны и павильона имеют противоположный знак угла наклона относительно
// плоскости OXY и удобно использовать для них разные функции.
Plane3D.prototype.CreateInclinePlane1 = function(angle, pt1, pt2, pt3)
{
	var norm2d = new Vector2D(pt1[1] - pt2[1], pt2[0] - pt1[0]);
	norm2d.Normer();

...
//	sp2.position.set(Y, 0, X);
	...
	{
		var rs = 0.4; // Sphere radius
		var sphereGeometry = new THREE.SphereGeometry(rs, 12, 12);
		var sphereMaterial = new THREE.MeshBasicMaterial({color: 0x550055});
		...
		sphere.position.x = tt.x;
		sphere.position.y = tt.y;
		sphere.position.z = tt.z;
		
		sphere_rotation.push(sphere);
		scene.add(sphere);
		///////////////////////////////////
		
		rs = 0.2; // Sphere radius
		sphereGeometry = new THREE.SphereGeometry(rs, 12, 12);
		...
		sphere_rotation_XY.push(sphere);
		scene.add(sphere);
...
{ 
	...
	ctx_hud_2.fillText("Quantum rotations", 50, 50);
